...
@SpringBootTest(classes = ChessserverApplication.class, webEnvironment = WebEnvironment.DEFINED_PORT)
public class SpringIntegrationTest {
    ...
    void executePost() throws IOException {
        ...
        latestResponse = restTemplate
          .execute("http://localhost:8080/user", HttpMethod.POST, requestCallback, response -> {
              if (errorHandler.hadError) {
                  return (errorHandler.getResults());
              } else {
                  return (new ResponseResults(response));
